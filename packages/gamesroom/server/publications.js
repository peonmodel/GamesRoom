...
// note since chat dont change within a room, it is fine
// doesnt work for games since the games will change
// the param passed in here determines whether it sees a change
...
Meteor.publish('CurrentRoom', function publishCurrent(roomId) {
	...
	let roomCursor;
	if (room.isPublic) {
		roomCursor = Room.collection.find({ _id: roomId });
	} else {
		roomCursor = Room.collection.find({ _id: roomId, members: this.userId });
	}
	// // const chatIds = roomCursor.map(o => o.chatId);
	// const gameIds = roomCursor.map(current => current.games).reduce((memo, array) => {
